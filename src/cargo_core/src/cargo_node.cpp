...
CargoNode::CargoNode():Node("cargo_node"){
    ...
    cargo_service = this->create_service<std_srvs::srv::Empty>(
        "/cargo_node/kill_all", 
        std::bind(&CargoNode::kill_node_callback, this, std::placeholders::_1, std::placeholders::_2)); 
    RCLCPP_INFO(this->get_logger(), "Cargo Node Initialized."); 

    RCLCPP_INFO(this->get_logger(), "Task Started."); 

}
void CargoNode::kill_node_callback(const std::shared_ptr<std_srvs::srv::Empty::Request> request, std::shared_ptr<std_srvs::srv::Empty::Response> response){
