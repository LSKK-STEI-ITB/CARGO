...
// TODO: Need safety check on each function at least to make sure it return a
// value.
...
void cargo_core::arm_drone(rclcpp::Node::SharedPtr nh, const ParamMap &params) {
  ...
  client = nh->create_client<mavros_msgs::srv::CommandBool>(
      "/" + nh->get_parameter("namespace").as_string() + "/cmd/arming");
  // Create request
  ...
  do {
    ...
    if (rclcpp::spin_until_future_complete(nh, future) ==
      ...
      int gotTopic = get_topic_val(
          nh, curr_state,
          "/" + nh->get_parameter("namespace").as_string() + "/state",
...
void cargo_core::get_alt(rclcpp::Node::SharedPtr nh, double &alt,
                         std::string source) {
  if (source == "local_pose") {
    std::string topic_name = "/" + nh->get_parameter("namespace").as_string() +
                             "/local_position/pose";
...
void cargo_core::switch_mode(rclcpp::Node::SharedPtr nh,
  ...
  setMode_client = nh->create_client<mavros_msgs::srv::SetMode>(
      "/" + nh->get_parameter("namespace").as_string() + "/set_mode");
...
template <class T>
bool cargo_core::get_topic_val(rclcpp::Node::SharedPtr nh, T &return_val,
...
void cargo_core::takeoff(rclcpp::Node::SharedPtr nh, const ParamMap &params) {
  ...
  takeoff_client = nh->create_client<mavros_msgs::srv::CommandTOL>(
      "/" + nh->get_parameter("namespace").as_string() + "/cmd/takeoff");
...
void cargo_core::move_local(rclcpp::Node::SharedPtr nh,
                            const ParamMap &params) {
  ...
  RCLCPP_INFO_STREAM(nh->get_logger(),
                     "--- Moving map-frame dx=" << dx_map << " dy=" << dy_map
                                                << " dz=" << dz_map << " ---");

  auto pub = nh->create_publisher<mavros_msgs::msg::PositionTarget>(
      "/" + nh->get_parameter("namespace").as_string() + "/setpoint_raw/local",
      10);

  geometry_msgs::msg::PoseStamped pose_start;
  get_topic_val(nh, pose_start,
  ...
  post_target.position.x = dx_map;
  post_target.position.y = dy_map;
  post_target.position.z = dz_map;
  ...
  while (rclcpp::ok()) {
    ...
    if (std::abs(dx_map - dx_now) < 0.1 && std::abs(dy_map - dy_now) < 0.1 &&
...
void cargo_core::initialize(rclcpp::Node::SharedPtr nh,
                            const ParamMap &params) {
  ...
  heading_sub = nh->create_subscription<std_msgs::msg::Float64>(
      ...
      rclcpp::SensorDataQoS(), headingCallback);
