...
void SafetyChecker::wait_for_message(std::shared_ptr<MessageT> &msg, rclcpp::Node::SharedPtr node,
                          const std::string &topic, 
                          std::chrono::nanoseconds timeout){
    auto sub = this->create_subscription<MessageT>(topic, rclcpp::QoS(10),[&msg](typename MessageT::SharedPtr m){msg = m;}); 
...
int SafetyChecker::rc_trigger(){
    ...
    wait_for_message<mavros_msgs::msg::RCIn>(rc_in_ptr, this->shared_from_this(), "/mavros/rc/in", std::chrono::seconds(5)); 
